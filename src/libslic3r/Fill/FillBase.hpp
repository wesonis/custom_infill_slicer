...
struct FillParams
{
    ...
    // For TPMS infills (Gyroid, Schwarz P, Schwarz D): per-axis wavelength multipliers (default 1.0 = unchanged).
    // X and Y act in the rotated infill frame (same convention as fill_angle).
    // Z is the world build-axis.
    float       tpms_period_x { 1.0f };
    float       tpms_period_y { 1.0f };
    float       tpms_period_z { 1.0f };
};
